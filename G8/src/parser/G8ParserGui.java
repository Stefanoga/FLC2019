...
public class G8ParserGui extends Parser {
	...
	public G8ParserGui(TokenStream input) {
		this(input, new RecognizerSharedState());
	}
	public G8ParserGui(TokenStream input, RecognizerSharedState state) {
		super(input, state);
	}

	@Override public String[] getTokenNames() { return G8ParserGui.tokenNames; }
	...
	public final void begin(String nameF, String dire) throws RecognitionException, IOException, SameNameError, ShapeLayoutError {
		this.name=nameF;
		this.dir=dire;
		Token b1=null;
		Token b2=null;
		...
		try {
			...
			G8.writeHTML("<!doctype html>", name, dir);
			G8.writeHTML("<html>", name, dir);
			G8.writeHTML("<head>", name, dir);
			G8.writeHTML("<title> " + Titolo + " </title>", name, dir);
			G8.writeHTML("<style> canvas {border: 1px #000 dotted;} </style>", name, dir);
			G8.writeHTML("<script>", name, dir);
			G8.writeHTML("window.onload = function () {\n", name, dir);
			G8.writeHTML("	var canvas = document.getElementById('" + Titolo + "');", name, dir);
			G8.writeHTML("	var context = canvas.getContext('2d'); \n", name, dir);
	...
	public final void circle() throws RecognitionException, IOException, SameNameError, ShapeLayoutError {
		...
		try {
			...
			switch (alt23) {
				...
				G8.writeHTML("	//" + Name, name, dir);
				G8.writeHTML("	context.beginPath();", name, dir);
				G8.writeHTML("	var centerX = " + xcenter + ";", name, dir);
				G8.writeHTML("	var centerY = " + ycenter + ";", name, dir);
				G8.writeHTML("	var radius = " + radius + ";", name, dir);
				G8.writeHTML("	var startAngle = " + startangle + "* Math.PI/180;", name, dir);
	...
	public final void ellipse() throws RecognitionException, IOException, SameNameError, ShapeLayoutError {
		...
		try {
			...
			switch (alt30) {
				...
				G8.writeHTML("	var centerX = " + xcenter + ";", name, dir);
				G8.writeHTML("	var centerY = " + ycenter + ";", name, dir);
				G8.writeHTML("	var radiusMax = " + semax + ";", name, dir);
				G8.writeHTML("	var radiusMin= " + semin + ";", name, dir);
				G8.writeHTML("	var rotation= " + rotation + "*Math.PI/180;", name, dir);
				G8.writeHTML("	var startAngle=" + startangle +"*Math.PI/180;", name, dir);
				G8.writeHTML("	var endAngle=" + endangle + "*Math.PI/180;", name, dir);
	...
	public final void end() throws RecognitionException, IOException {
		try {
			...
			G8.writeHTML("}", name, dir);
			G8.writeHTML("</script>", name, dir);
			G8.writeHTML("</head>", name, dir);
			G8.writeHTML("<body>", name, dir);
			G8.writeHTML("<canvas id='" + Titolo + "' width='" + DrawspaceWidth + "' height='" + DrawspaceHeigth + "'></canvas>", name, dir);
			G8.writeHTML("</body>", name, dir);
			G8.writeHTML("</html>", name, dir);
